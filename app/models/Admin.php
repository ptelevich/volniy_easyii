...
class Admin extends \yii\easyii\models\Admin implements \yii\web\IdentityInterface
{
    const RULE_ROOT = 'superadmin';

    static $roles = [
        self::RULE_ROOT => 'IS_ROOT',
        'seo' => 'IS_SEO'
    ];
    ...
    public function rules()
    {
        return [
            ...
            [['password', 'role'], 'safe'],
            ['role', 'string'],
            [['access_token', 'role'], 'default', 'value' => null]
        ];
    }
    ...
    public static function findIdentity($id)
    {
        ...
        foreach(self::$roles as $role => $constName) {
            if($role == self::RULE_ROOT) {
                continue;
            }
            defined($constName) or define($constName, isset(self::$roles[$result->role]));
        }
    ...
    {
        return (
            $this->username === self::$rootUser['username'] ||
            $this->role === self::RULE_ROOT
        );
    }
}
